...
export default function Profile() {
  ...
  const fetchUserData = async () => {
    ...
    let pUserId = null;
    const { data: profiles } = await supabase.from('profiles').select('id, email').ilike('email', `${username}@%`).limit(1);
  ...
  const hasVerifiedBadge = badges.some(b => b.badge_name === 'Rater #1');
